...
export const SearchHistory:FC = () => {
    ...
    return (
        <FlatList
          ...
          renderItem={({ item }) => (
            <HorizontalMediaCardWithLink name={item.title} poster_path={item.posterUrl} id={item.id} overview={item.overview} type={item.media_type.mediaType} vote_average={item.vote_average} />
          )}
        />
    )
}