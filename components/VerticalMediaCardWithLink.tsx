    ...
    if (mediaType === 'movie') {
        return (
            <Link href={`/movies/${id}`} asChild className="py-4">
                ...
                <VerticalMediaCard title={title} release_date={release_date} vote_average={vote_average} overview={overview} poster_path={poster_path} type={type} />
                </TouchableOpacity>
            </Link>
        )
        ...
        return (
            <Link href={`/tv/${id}`} asChild className="py-4">
                <TouchableOpacity className="w-28 h-56 flex-1 my-6">
                <VerticalMediaCard title={title} poster_path={poster_path} vote_average={vote_average} type={type} overview={overview} />
                </TouchableOpacity>
            </Link>
        )
    }
